...
func main() {
	...
	noRoundPtr := flag.Int("rounds", 3, "number of rounds")
	flag.Parse()
    ...
    secretNumber := randomizer.Intn(upperLimit)  // generate numbers between 0 and 10 only

    // we declare a variable of type int called "guess"
    var guess int

    // create a for loop that begins at 1 and loops to 3
    for try := 1; try <= *noRoundPtr; try++ {
